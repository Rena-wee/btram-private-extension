...
import {
    ...
    URLBuilder,
    MDImageQuality
} from './YuriGardenHelper'

import {
    parseMangaList
} from './YuriGardenParser'

import tagJSON from './external/tag.json'
import { CuuTruyenSearchResponse } from './YuriGardenInterfaces'
...
export class MangaDex implements ChapterProviding, SearchResultsProviding, HomePageSectionsProviding {
    MANGADEX_DOMAIN = CUUTRUYEN_DOMAIN
    MANGADEX_API = CUUTRUYEN_API
        ...
        for (const section of sections) {
            ...
            promises.push(
                this.requestManager.schedule(section.request, 1).then(async (response) => {
                    const json: CuuTruyenSearchResponse = (typeof response.data === 'string') ? JSON.parse(response.data) : response.data
