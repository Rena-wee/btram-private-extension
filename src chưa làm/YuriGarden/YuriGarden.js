...
const YuriGardenSettings_1 = require("./YuriGardenSettings");
...
class MangaDex {
        ...
        this.requestManager = App.createRequestManager({
            ...
            interceptor: {
                interceptRequest: async (request) => {
                    ...
                    let accessToken = await (0, YuriGardenSettings_1.getAccessToken)(this.stateManager);
                    ...
                    if (Number(accessToken.tokenBody.exp) <= Date.now() / 1000 - 60) {
                        try {
                            const response = await (0, YuriGardenSettings_1.authEndpointRequest)(this.requestManager, 'refresh', {
                                token: accessToken.refreshToken
                            });
                            accessToken = await (0, YuriGardenSettings_1.saveAccessToken)(this.stateManager, response.token.session, response.token.refresh);
                            if (!accessToken)
                                return request;
                        }
                        catch {
    ...
    async getSourceMenu() {
        return App.createDUISection({
            ...
            isHidden: false,
            rows: async () => [
                await (0, YuriGardenSettings_1.accountSettings)(this.stateManager, this.requestManager),
                (0, YuriGardenSettings_1.contentSettings)(this.stateManager),
                (0, YuriGardenSettings_1.thumbnailSettings)(this.stateManager),
                (0, YuriGardenSettings_1.resetSettings)(this.stateManager)
            ]
        });
    }
    ...
    async getCustomListRequestURL(listId, ratings) {
        ...
        return new YuriGardenHelper_1.URLBuilder(this.MANGADEX_API)
    ...
    async getMangaDetails(mangaId) {
        ...
        const request = App.createRequest({
            url: new YuriGardenHelper_1.URLBuilder(this.MANGADEX_API)
                .addPathComponent('manga')
        ...
        if (coverFileName) {
            image = `${this.COVER_BASE_URL}/${mangaId}/${coverFileName}${YuriGardenHelper_1.MDImageQuality.getEnding(await (0, YuriGardenSettings_1.getMangaThumbnail)(this.stateManager))}`;
        }
    ...
    async getChapters(mangaId) {
        this.checkId(mangaId);
        const languages = await (0, YuriGardenSettings_1.getLanguages)(this.stateManager);
        const skipSameChapter = await (0, YuriGardenSettings_1.getSkipSameChapter)(this.stateManager);
        const ratings = await (0, YuriGardenSettings_1.getRatings)(this.stateManager);
        ...
        while (hasResults) {
            ...
            for (const chapter of json.data) {
                ...
                const langCode = YuriGardenHelper_1.MDLanguages.getFlagCode(chapterDetails.translatedLanguage);
    ...
    async getChapterDetails(mangaId, chapterId) {
        ...
        const dataSaver = await (0, YuriGardenSettings_1.getDataSaver)(this.stateManager);
        const forcePort = await (0, YuriGardenSettings_1.forcePort443)(this.stateManager);
        const request = App.createRequest({
    ...
    async getSearchResults(query, metadata) {
        ...
        const url = new YuriGardenHelper_1.URLBuilder(this.MANGADEX_API)
            .addPathComponent('manga')
        ...
        results = await (0, YuriGardenParser_1.parseMangaList)(json.data, this, YuriGardenSettings_1.getSearchThumbnail);
    ...
    async getHomePageSections(sectionCallback) {
        ...
        for (const section of sections) {
            ...
            promises.push(this.requestManager.schedule(section.request, 1).then(async (response) => {
                ...
                section.section.items = await (0, YuriGardenParser_1.parseMangaList)(json.data, this, YuriGardenSettings_1.getHomepageThumbnail);
                sectionCallback(section.section);
            }));
        }
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
        ...
        results = await (0, YuriGardenParser_1.parseMangaList)(json.data, this, YuriGardenSettings_1.getHomepageThumbnail);
        return App.createPagedResults({
            results,
...
//# sourceMappingURL=YuriGarden.js.map